...
def LASSO_all_genes(protein_genes, y_full_df, ancsetry, bfile=None, alpha=0.1, test_size=0.2, random_state=42, save_snps=True):
    results = {}
    snps_list = []
    for gene_id in tqdm(protein_genes["gene_id"]):
        ...
        if save_snps:
            ...
            snps.to_csv(f"./project_data/results/effect/{ancsetry}/LASSO_{gene_id}_effect_weights.csv")
